...
```{r load3}
mix <- rbind(tr, te)
mix <- mix[, c("var_0",  "var_2",  "var_6",  "var_80", "var_81",  "var_139", "var_191", "var_12",  "var_40",  "var_164",  "var_177",  "var_188", "var_9", "var_13", "var_108",  "var_184")]

mixbin <- bin(mix, nbins=100)
```

```{r}
mixbin <- one_hot(mixbin)
```

```{r}
mixbin <- prcomp_irlba(mixbin, n=200)

bin_tr <- mixbin$x[1:200000, ]
bin_te <- mixbin$x[200001:400000, ]
```

```{r}
train <- cbind(tr, bin_tr)
test <- cbind(te, bin_te)

rm(mixbin, bin_tr, bin_te);gc();gc()
```
