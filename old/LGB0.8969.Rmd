...
```{r load2}
tr2 <- fread("Kolmogorov–Smirnov/ex_train.csv", header = T, showProgress = F)
te2 <- fread("Kolmogorov–Smirnov/ex_test.csv", header = T, showProgress = F)
```
...
tr_cumsum <- cumsum(tr2)
te_cumsum <- cumsum(te2)
...
tr_digit1 <- tr2 %>% round(1)
te_digit1 <- te2 %>% round(1)

tr_digit2 <- tr2 %>% round(2)
te_digit2 <- te2 %>% round(2)
```


```{r factor}
tr2$f_var_68 <- as.factor(tr2$var_68) %>%
  fct_lump(prop = 0.001)

te2$f_var_68 <- as.factor(te2$var_68) %>%
  fct_lump(prop = 0.001)
...
```{r load3}
tr_r <- fread("Kolmogorov–Smirnov/ex_train.csv", header = T, showProgress = F)
te_r <- fread("Kolmogorov–Smirnov/ex_test.csv", header = T, showProgress = F)
```
